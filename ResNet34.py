...
class Block(nn.Module):
 ...
 super().__init__()
 self.conv1 = nn.Conv2d(dim, out_dim//4, kernel_size=1, stride=stride, bias=False) # 修改为1x1卷积，并减少输出通道数
 self.bn1 = nn.BatchNorm2d(out_dim//4)
 self.relu1 = nn.ReLU()
 self.conv2 = nn.Conv2d(out_dim//4, out_dim//4, kernel_size=3, padding=1, bias=False) # 保持3x3卷积，但减少输入和输出通道数
 self.bn2 = nn.BatchNorm2d(out_dim//4)
 self.relu2 = nn.ReLU()
 self.conv3 = nn.Conv2d(out_dim//4, out_dim, kernel_size=1, bias=False) # 增加一个1x1卷积，恢复输出通道数
